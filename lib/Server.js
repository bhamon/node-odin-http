	...
	*/
	listen(p_port, p_host) {
		...
		return new Promise(function(p_resolve, p_reject) {
			self[SYMBOL_MEMBER_SERVER].on('listening', p_resolve);
			self[SYMBOL_MEMBER_SERVER].on('error', p_reject);

			self[SYMBOL_MEMBER_SERVER].listen(args.port, args.host);
		});
	}
	...
	*/
	shutdown() {
		...
		let promise = new Promise(function(p_resolve) {
			self[SYMBOL_MEMBER_SERVER].close(p_resolve);
		});
