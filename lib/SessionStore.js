	...
	*/
	[SYMBOL_METHOD_CREATE_UNIQUE_TOKEN]() {
		return lib.deps.co(function*() {
			let bytes = yield lib.odin.util.promise.nfcall(lib.node.crypto.randomBytes, this.tokenSize);
	...
	*/
	create() {
		...
		return lib.deps.co(function*() {
			let token;
			do {
				token = yield this[SYMBOL_METHOD_CREATE_UNIQUE_TOKEN](self.tokenSize, self[SYMBOL_MEMBER_MAP]);
