...
class SessionStore {
	...
	*/
	constructor(p_config) {
		...
		*/
		Object.defineProperty(this, SYMBOL_MEMBER_MAP, {writable:true, value:{}});
	...
	*/
	[SYMBOL_METHOD_CREATE_UNIQUE_TOKEN]() {
		return lib.odin.util.promise.nfcall(lib.node.crypto.randomBytes, this.tokenSize)
		.then(function(p_bytes) {
			let token = p_bytes.toString('hex');
	...
	*/
	create() {
		let self = this;
		return this[SYMBOL_METHOD_CREATE_UNIQUE_TOKEN](self.tokenSize, self[SYMBOL_MEMBER_MAP])
	...
	*/
	removeExpiredSessions(p_expireAfter) {
		...
		.filter(function(p_session) {
			return session.hasExpired(nom, p_expireAfter);
		})
