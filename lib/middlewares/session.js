...
module.exports = function(p_request, p_response, p_next) {
	...
	*/
	p_request.initSession = function(p_sessionStore) {
		...
		try {
			lib.registry.sessionsStore.checkToken(p_request.auth.token);
		} catch(p_ex) {
			throw new lib.odin.http.ExceptionRest(409, 'auth.validation', 'Malformed token', {}, p_ex);
		}

		p_request.session = lib.registry.sessionsStore.get(p_request.auth.token);
