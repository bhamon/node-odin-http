...
/**
	...
	@throws {module:odin/http.ExceptionRest}					[401 :: auth.unknownSession] If there is no matching session for the provided token.
...
function initSession(p_request, p_sessionStore) {
	...
	if(!p_request.session) {
		throw new lib.odin.http.ExceptionRest(401, 'auth.unknownSession', 'Unknown session');
	}
