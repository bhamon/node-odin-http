...
class Router {
	...
	*/
	attachMiddlewares(p_path) {
		let self = this;
		return lib.odin.util.promise.nfcall(lib.node.fs.readdir, p_path)
		.then(function(p_files) {
			return p_files
			.filter(function(p_file) { return lib.node.path.extname(p_file) == '.js'; })
			.map(function(p_file) {
				let middleware = require(lib.node.path.join(p_path, p_file));
				self.use(middleware);
			});
		});
	}
	...
	attachRouters(p_path) {
		...
		.then(function(p_files) {
			...
			return Promise.all(
				...
				.filter(function(p_file) { return lib.node.path.extname(p_file) == '.js'; })
				.map(function(p_file) {
					let router = require(lib.node.path.join(p_path, p_file));
